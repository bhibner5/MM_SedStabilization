---
title: 'wokring with microbial mat data '
output:
  html_document:
    df_print: paged
---
...
The master sheet is all of my ooid data from any mat experiment tested, the sheet is different for each exp but has the same headers/information
...
looks like the weird increase is almost within the standard deviation of the middle low day so it could be true that it just levels off. when looking through photos the white (ooids) seems to spead out so maybe ooid 'chunks' are spreading from a pile up to a more flat surface

we can test this by maybe putting a little pile out and measuring its spread over the week in the july trip?

exp- take photos from same height and with a scale and then draw circle around edge to see how it changes over the week
...
## some question and exp ideas I have

*is filament growth light dependent??*

could test this using electrical tape/ or a cut lid (might be better) on one side of the each of the control experiments, do two again (one with beach sand and one with sieved sand

*is all parts of the island have the same amount of sediment on top?*

use the cut out for the long term experiments and take a 'cross section' of standard sediment from beach into interior (minimum ten locations) to see if there is an increase as you are closer to the edge etc.

same- one mechanism is delivering all the sediment and it may just be storms

different- I would say tides push it in if there is more closer to the beach

look at size of sediment -- determine if wind or water is moving it based on size
